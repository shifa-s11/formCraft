...
//         <>
//           <p className="mt-1 font-semibold text-gray-400">Passage:</p>
...
import { CSS } from "@dnd-kit/utilities";
import { Button } from "@/components/ui/button";
import { Trash, ArrowUp, ArrowDown, GripVertical, Pencil } from "lucide-react";
import type { Question } from "@/lib/questionApi";
    ...
    <div ref={setNodeRef} style={style} className="bg-gray-800 border border-gray-700 rounded-lg p-4 mb-3 flex flex-col gap-3">
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-2 text-white">
          <Button onClick={() => onEdit(question)} variant="ghost" size="icon">
            <Pencil size={16} />
          </Button>
          <Button onClick={onMoveUp} disabled={isFirst} variant="ghost" size="icon">
            <ArrowUp size={16} />
          </Button>
          <Button onClick={onMoveDown} disabled={isLast} variant="ghost" size="icon">
            <ArrowDown size={16} />
          </Button>
          <Button onClick={() => onDelete(question._id!)} variant="ghost" size="icon">
...
export default function SortableQuestionsList({ questions, setQuestions, onEdit, onDelete }: SortableQuestionsListProps) {
  const handleDragEnd = (event: DragEndEvent) => {
    const { active, over } = event;
    if (!over || active.id === over.id) return;
